...
-- Consulta 1
/*
El cliente nos solicita un pequeño reporte de pruebas donde se registren el
...
/*
SELECT v.viajero_id, v.nombre, 
	t.numero_boleto,
	d.nombre_destino
FROM viajeros v
	LEFT JOIN tickets t ON t.viajero_id = v.viajero_id
	LEFT JOIN destinos d ON d.destino_id = t.destino_id
*/

-- Manejando la salida de los valores null
/*
SELECT v.viajero_id, v.nombre, 
	COALESCE(t.numero_boleto, '--') numero_boleto,
	COALESCE(d.nombre_destino, '--') nombre_destino
FROM viajeros v
	LEFT JOIN tickets t ON t.viajero_id = v.viajero_id
	LEFT JOIN destinos d ON d.destino_id = t.destino_id
*/
...
-- El boleto: T123456 no existe, para realizar la practica
-- usaremos un boleto que si existe
/*
SELECT t.*, v.*, d.*
FROM tickets t
	LEFT JOIN viajeros v ON v.viajero_id = t.viajero_id
	LEFT JOIN destinos d ON d.destino_id = t.destino_id
WHERE t.numero_boleto = 'T111111'
*/
...
/*
SELECT v.viajero_id, v.nombre, 
	t.numero_boleto, t.fecha_salida, t.fecha_retorno,
	d.nombre_destino
FROM viajeros v
	LEFT JOIN tickets t ON t.viajero_id = v.viajero_id
	LEFT JOIN destinos d ON d.destino_id = t.destino_id
WHERE t.fecha_salida = '2024-01-10'
	OR t.fecha_retorno = '2024-01-10'
*/

-- Consulta 4
-- Obtener el número total de boletos por cada género
/*
SELECT v.genero, COUNT(t.*) total_boletos
FROM tickets t
	LEFT JOIN viajeros v ON v.viajero_id = t.viajero_id
GROUP BY v.genero
*/
...
FROM viajeros v
	LEFT JOIN tickets t ON t.viajero_id = v.viajero_id
	LEFT JOIN destinos d ON d.destino_id = t.destino_id
WHERE d.nombre_destino = 'Playa del Carmen'
*/
...
FROM viajeros v
	LEFT JOIN tickets t ON t.viajero_id = v.viajero_id
	LEFT JOIN destinos d ON d.destino_id = t.destino_id
WHERE d.nombre_destino LIKE 'Playa%Carmen'
*/

-- Si deseamos saber los clientes que han viajado a una playa
/*
SELECT v.viajero_id, v.nombre, 
	t.numero_boleto, t.fecha_salida, t.fecha_retorno,
	d.nombre_destino
FROM viajeros v
	LEFT JOIN tickets t ON t.viajero_id = v.viajero_id
	LEFT JOIN destinos d ON d.destino_id = t.destino_id
WHERE d.nombre_destino LIKE '%Playa%'
*/
